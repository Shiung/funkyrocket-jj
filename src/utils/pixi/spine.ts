...
export function playSpineAnimationWithTrack(
  ...
  try {
    ...
    logger?.(`🎬 新增軌道動畫: ${animationName} (循環: ${loop})`)
    return animation
  } catch (error) {
    logger?.(`❌ 播放動畫失敗: ${error}`)
    return null
...
export function setSpineAnimationSpeed(
  ...
  try {
    ...
    logger?.(`⏱️ 動畫速度設置為: ${speed}x`)
  } catch (error) {
    logger?.(`❌ 設置動畫速度失敗: ${error}`)
  }
}
...
export function applySpineTransform(
  ...
  try {
    ...
    // logger?.(`🔄 Spine 變換已應用: 位置(${spine.x}, ${spine.y}), 縮放(${spine.scale.x}, ${spine.scale.y}), 旋轉(${spine.rotation})`)
  } catch (error) {
    logger?.(`❌ 應用 Spine 變換失敗: ${error}`)
  }
}
...
export function clearSpineStateWithTrack(spine: Spine, track: number, logger?: (message: string) => void): void {
  ...
  try {
    ...
    logger?.(`🧹 Spine 動畫狀態已清理`)
  } catch (error) {
    logger?.(`❌ 清理 Spine 動畫狀態失敗: ${error}`)
  }
}
