...
export const useBackground = (getApp: () => any) => {
  // 基礎配置
  const { gameWidth, gameHeight, MIN_GAME_WIDTH, MAX_GAME_WIDTH, baseOffsetY, isDesktop } =
    useBaseConfig()
  // 背景精靈實例
  ...
  const cycleBackground = computed(() => '/assets/images/bg/cycle.webp')

  const frontCloud = computed(() => '/assets/images/bg/front_cloud.webp')

  const pageBackgroundImage = computed(() => `url('/assets/images/bg/pc.webp')`)
  ...
  const updateMobileFrontCloudScale = (): void => {
    ...
    if (gameWidth.value >= MIN_GAME_WIDTH && gameWidth.value <= MAX_GAME_WIDTH) {
      ...
      console.log(
        `375基準縮放模式 - 螢幕寬度: ${gameWidth.value}px, 縮放係數: ${scaleFactor.toFixed(3)}, 最終縮放: ${finalScale.toFixed(3)}, 雲朵寬度: ${scaledWidth.toFixed(0)}px`,
      )
    } else {
      // 450以上：使用預設處理
    ...
    const scroll = () => {
      ...
      cycleBackgroundSprites.forEach((sprite) => {
        ...
        if (sprite.y > gameHeight.value + sprite.height) {
          ...
          const allSprites = [defaultBackgroundSprite, ...cycleBackgroundSprites].filter(
            (s) => s !== null && s !== sprite,
          )
  ...
  const updateMobileBackgroundScale = (): void => {
    if (defaultBackgroundSprite) {
      ...
      console.log(
        `背景最終尺寸: ${(texture.width * scale).toFixed(0)} x ${(texture.height * scale).toFixed(0)}`,
      )
